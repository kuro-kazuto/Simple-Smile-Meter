...
This is a simple program to measure your smile using open cv.
Created By : GALIH AJI PAMBUDI
"""
...
def smile_meter(frame, x1, y1):
    ...
    if num>4000:
        ...
        color = (255, 0, 255)
        text = cv2.putText(frame, "Senyum Anda Sebesar", (int(x1)+15, int(y1)-70), font, 1, color, 4, cv2.LINE_AA)
...
while True:
    ...
    for x, y, w, h in face:
        img = cv2.rectangle(frame, (x, y), (x+(y+20), y+(h-300)), (0, 0, 255), -1)
    ...
    cv2.imshow("Parameter Senyum", frame)
    key = cv2.waitKey(1)
    if key == ord('q'):
        break
